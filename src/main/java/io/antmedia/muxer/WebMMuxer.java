...
public class WebMMuxer extends Muxer {
	...
	/**
	 * Dynamic means that this webm muxer is added on the fly. 
	...
	private boolean isCodecSupported(int codecId) {
		for (int i=0; i< WEBM_SUPPORTED_CODECS.length; i++) {
			if (codecId == WEBM_SUPPORTED_CODECS[i]) {
				return true;
			}
		}
		return false;
	...
	public void writeVideoBuffer(ByteBuffer encodedVideoFrame, long timestamp, int frameRotation, int streamIndex,boolean isKeyFrame,long firstFrameTimeStamp) {
		...
		/*
		* Rotation field is used add metadata to the webm.
		*/
	...
	public synchronized void writeTrailer() {
		...
		outputFormatContext.streams(videoIndex).metadata(metadata);
		...
		scheduler.addScheduledOnceJob(0, new IScheduledJob() {
			...
			public void execute(ISchedulingService service) throws CloneNotSupportedException {
				try {
					...
					logger.info("WebM {} is ready", f.getName());

					IContext context = WebMMuxer.this.scope.getContext();
