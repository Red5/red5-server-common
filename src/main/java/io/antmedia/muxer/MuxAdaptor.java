...
public class MuxAdaptor implements IRecordingListener {
	...
	public void writeStreamPacket(IStreamPacket packet) {
		...
		if (packet.getDataType() == Constants.TYPE_VIDEO_DATA) 
		{
			...
			{
				for (Muxer muxer : muxerList) {
					muxer.writeVideoBuffer(byteBuffer, packet.getTimestamp(), 0, videoStreamIndex, (frameType & 0xF0) == IVideoStreamCodec.FLV_FRAME_KEY, 0, packet.getTimestamp()+compositionTimeOffset);
				}
			}
